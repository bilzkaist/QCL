...
def preprocess_data(data, window_size=20):
    print(f"Preprocessing WAADM data with window size {window_size}...")
    ...
    # Process WAADM dataset based on 'signal_strength' column
    if 'signal_strength' in data.columns:
        print("Processing WAADM data from 'signal_strength'...")
        data['signal_strength'] = scaler.fit_transform(data['signal_strength'].values.reshape(-1, 1))
        X = np.array([data['signal_strength'].values[i:i + window_size] for i in range(len(data) - window_size)])
        y_true = np.array([1 if val > 0.8 else 0 for val in data['signal_strength'][window_size:]])
    else:
...
def load_datasets():
    ...
    datasets['WAADM'] = data_waadm
    print("Loaded WAADM dataset.")
    visualize_dataset(data_waadm, 'WAADM')
...
def visualize_dataset(data, dataset_name):
    ...
    # Visualize WAADM dataset based on 'signal_strength' column
    if 'signal_strength' in data.columns:
        plt.plot(data.index, data['signal_strength'], label='Signal Strength')
